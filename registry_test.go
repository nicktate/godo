...
func TestRepository_ListImages(t *testing.T) {
	setup()
	defer teardown()

	wantImages := []*RepositoryImage{
		{
			RegistryName:        "foo",
	...
	getResponseJSON := `
{
	...
	"links": {
	    "pages": {
			"next": "https://api.digitalocean.com/v2/registry/foo/repositories/repo-name/images?page=2",
			"last": "https://api.digitalocean.com/v2/registry/foo/repositories/repo-name/images?page=2"
		}
	},
	"meta": {
	    "total": 2
	...
	mux.HandleFunc("/v2/registry/foo/repositories/repo-name/images", func(w http.ResponseWriter, r *http.Request) {
		testMethod(t, r, http.MethodGet)
		testFormValues(t, r, map[string]string{"page": "1", "per_page": "1"})
		fmt.Fprint(w, getResponseJSON)
	})
	got, response, err := client.Registry.ListRepositoryImages(ctx, &RepositoryListImagesRequest{RegistryName: "foo", Repository: "repo-name"}, &ListOptions{Page: 1, PerPage: 1})
	require.NoError(t, err)
	require.Equal(t, wantImages, got)
